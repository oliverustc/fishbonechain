...
.PHONY: build build-release build-node build-runtime build-2s build-1s build-10mb build-babe check test clean
...
build-2s:
	mkdir -p deploy/bin
	WASM_BUILD_RUSTFLAGS="$(WASM_FLAGS)" cargo build --release -p fishbone-node --features fishbone-runtime/block-2s
	cp target/release/fishbone-node deploy/bin/fishbone-node-2s

build-1s:
	mkdir -p deploy/bin
	WASM_BUILD_RUSTFLAGS="$(WASM_FLAGS)" cargo build --release -p fishbone-node --features fishbone-runtime/block-1s
	cp target/release/fishbone-node deploy/bin/fishbone-node-1s

# 大区块变体（子链 3：医疗影像标注，10 MB 区块）
build-10mb:
	mkdir -p deploy/bin
	WASM_BUILD_RUSTFLAGS="$(WASM_FLAGS)" cargo build --release -p fishbone-node --features fishbone-runtime/block-10mb
...
build-babe:
	mkdir -p deploy/bin
	WASM_BUILD_RUSTFLAGS="$(WASM_FLAGS)" cargo build --release -p fishbone-node --features fishbone-runtime/babe
