...
/// @title PianoVerifier
/// @notice Piano 协议的链上 Solidity 验证合约。
///
...
contract PianoVerifier {
    ...
    /// @notice 验证密钥，与 BPianoVerifier 相同的 VK 结构。
    struct VerifyingKey {
        // 选择子多项式全局承诺（G1）
        Pairing.G1Point ql;
        Pairing.G1Point qr;
    ...
    function _verifyX(
        ...
        uint256 alphaPowT  = _modexp(ch.alpha, vk.sizeX, FR);
        ...
        return _pairing2(LHS, vk.g2_0, Pairing.ecNeg(RHS), vk.g2_1);
    }
    ...
    function _computeFoldedXPoints(
        ...
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.ql, gammaPow[4]));
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.qr, gammaPow[5]));
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.qm, gammaPow[6]));
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.qo, gammaPow[7]));
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.qk, gammaPow[8]));
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.s1, gammaPow[9]));
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.s2, gammaPow[10]));
        foldedComF = Pairing.ecAdd(foldedComF, Pairing.ecMul(vk.s3, gammaPow[11]));
    ...
    function _verifyY(
        ...
        uint256 betaPowM  = _modexp(ch.beta, vk.sizeY, FR);
        ...
        return _pairing2(diff, vk.g2y_0, Pairing.ecNeg(p.batchYH), tauYBetaG2);
    }
