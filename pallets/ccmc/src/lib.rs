...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn register_child_chain(
			...
			let miner_info = MinerInfo { deposit: deposit_required, status: MinerStatus::Active };
		...
		pub fn join_child_chain(origin: OriginFor<T>, chain_id: ChainId) -> DispatchResult {
			let who = ensure_signed(origin)?;
			...
			let miner_info =
				MinerInfo { deposit: chain.deposit_required, status: MinerStatus::Active };
			Miners::<T>::insert(chain_id, &who, miner_info);
		...
		pub fn leave_child_chain(origin: OriginFor<T>, chain_id: ChainId) -> DispatchResult {
			let who = ensure_signed(origin)?;
		...
		pub fn terminate_child_chain(origin: OriginFor<T>, chain_id: ChainId) -> DispatchResult {
			let mut chain = ChildChains::<T>::get(chain_id).ok_or(Error::<T>::ChainNotFound)?;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn miner_count(chain_id: ChainId) -> u32 {
			ChildChains::<T>::get(chain_id).map(|c| c.miner_count).unwrap_or(0)
		}
