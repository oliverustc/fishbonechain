...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn submit_bill(
			...
			ensure!(T::CcmcPallet::is_miner(task.target_chain, &who), Error::<T>::NotAMiner);

			// 验证账单总额不超预算
			let total: BalanceOf<T> =
				bill_amounts.iter().try_fold(BalanceOf::<T>::default(), |acc, (_, amount)| {
