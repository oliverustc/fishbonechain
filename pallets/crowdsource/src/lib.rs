...
pub mod pallet {
    ...
    impl WeightInfo for () {
        fn sync_task()       -> Weight { Weight::from_parts(10_000, 0) }
        fn submit_data()     -> Weight { Weight::from_parts(15_000, 0) }
        fn finalize_epoch()  -> Weight { Weight::from_parts(50_000, 0) }
    }
    ...
    pub trait Config: frame_system::Config {
        ...
        /// 本子链在主链 CCMC 中注册的 ChainId（通过 chain spec / parameter_types 注入）
        #[pallet::constant]
        ...
        fn do_finalize_epoch() {
            ...
            let root: T::Hash = if submissions.is_empty() {
                ...
                let leaves: Vec<T::Hash> = submissions
                    .iter()
                    .map(|s| <T::Hashing as HashT>::hash(&s.encode()))
                    .collect();
