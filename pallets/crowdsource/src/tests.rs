...
fn collecting_slot_transitions_to_syncing_at_block_100() {
	new_test_ext().execute_with(|| {
		...
		System::assert_has_event(Event::SyncingSlotStarted { epoch: 0, block: 100 }.into());
	});
}
...
#[test]
fn emits_configured_chain_id() {
	new_test_ext().execute_with(|| {
		run_to_block(120);

		let events = System::events();
		let chain_id = events.iter().find_map(|e| {
			if let RuntimeEvent::Crowdsource(Event::EpochFinalized { chain_id, .. }) =
				e.event.clone()
			{
				Some(chain_id)
			} else {
...
fn budget_exhaustion_marks_task_and_rejects_further_submissions() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(crate::ActiveTasks::<Test>::get(0).unwrap().status, TaskStatus::Exhausted);
		// 后续提交应失败
		assert_noop!(
			Crowdsource::submit_data(
				RuntimeOrigin::signed(3),
				0,
				b"y".to_vec().try_into().unwrap(),
				1
			),
...
fn bill_amounts_aggregated_per_worker() {
	new_test_ext().execute_with(|| {
		...
		let finalized = events
			.iter()
			.find_map(|e| {
				if let RuntimeEvent::Crowdsource(Event::EpochFinalized { bill_amounts, .. }) =
					&e.event
				{
		...
		let bill_map: std::collections::HashMap<u64, u64> = finalized.iter().cloned().collect();
...
fn epoch_resets_exhausted_tasks_after_finalization() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(crate::ActiveTasks::<Test>::get(0).unwrap().status, TaskStatus::Active);
	});
}
