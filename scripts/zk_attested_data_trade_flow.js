 ...
 *
 * 与 data_trade_flow.js 相同但输出 verifier=zk-attested，用于区分 mock 和 attestation 模式。
 *
 * 用法：
...
async function main() {
  ...
  try {
    ...
    for (let round = 0; round < 2; round++) {
      ...
      await submitTx(bob, childApi.tx.tradeSession.submitDataProof(sessionId, round, ch), `submitDataProof(${round})`);
      // ZK attestation by verifier (Charlie)
      await submitTx(charlie, childApi.tx.tradeSession.attestDataProof(sessionId, round, ch, true), `attestDataProof(${round})`);
