...
为更直接展示“单链混跑”和“多子链隔离”的差异，补充整理跨场景隔离对比。单链混跑数据来自实验 A 的 worker 统计；多子链隔离列采用“独享 epoch 容量归一化”口径，表示各场景不再互相争抢同一条链的 `MaxSubmissionsPerEpoch`。

![跨场景隔离效果对比](figures/fig_isolation_comparison.png)

| 场景 | 单链混跑成功率 | 多子链隔离归一化 | 改善 |
|------|---------------:|-----------------:|-----:|
| a 快递配送 | 12.46% | 100% | 8.03× |
| b 交通感知 | 10.67% | 100% | 9.37× |
| c 医疗标注 | 15.26% | 100% | 6.55× |
| e 传感器网络 | 1.37% | 100% | 73.13× |

> 口径说明：隔离列的 100% 表示场景独享子链容量，不再被其他场景挤占；它不是宣称长期运行中完全没有链上约束拒绝。详细说明见 `isolation-experiment.md`。
...
为进一步说明主链不会随 worker 高频提交同步承压，补充执行 N=1..6 子链线性扩展实验。该实验使用统一 workload：每条活跃子链 50 workers、0.005 req/s、800B payload、0.001 UNIT 奖励。奖励口径低于场景 d 的 50 UNIT，是为了适配 child2 和 child5 当前任务预算，避免小预算链提前耗尽导致吞吐不可比。

![子链吞吐线性扩展与主链负载](figures/fig_scale_mainchain_load.png)

**图 6b**：N=1..6 条子链并发时的聚合 TPS 与主链负载。左图展示子链聚合成功提交 TPS 接近线性增长；右图展示主链桥接 TPS、主链总 TPS，以及桥接 TPS 相对子链 TPS 的占比。

| N | 子链聚合 TPS | 主链桥接 TPS | 主链总 TPS | 桥接/子链吞吐占比 |
|---|-------------:|-------------:|-----------:|------------------:|
| 1 | 0.2069 | 0.0025 | 0.1695 | 1.23% |
| 2 | 0.4056 | 0.0056 | 0.1725 | 1.38% |
| 3 | 0.6203 | 0.0081 | 0.1751 | 1.31% |
| 4 | 0.8251 | 0.0112 | 0.1781 | 1.35% |
| 5 | 1.0025 | 0.0146 | 0.1814 | 1.46% |
| 6 | **1.2189** | **0.0202** | **0.1870** | **1.66%** |

**关键观察**：
- 当子链数量从 1 增加到 6 时，聚合吞吐量从 0.2069 TPS 增加到 1.2189 TPS，达到 5.89×。
- 同期主链桥接负载仅从 0.0025 TPS 增加到 0.0202 TPS。
- 在 6 条子链同时运行时，主链桥接 TPS 仅为子链聚合提交 TPS 的 1.66%，说明高频 worker 提交被子链吸收，主链只承担低频摘要和结算交易。

> 口径说明：`主链总 TPS` 是观测到的主链 extrinsics 速率，包含区块基础 inherent 等非业务交易；`桥接/子链吞吐占比` 是 `main_bridge_tps / child_subs_tps`，不是主链最大容量占用率。若需宣称主链容量占用百分比，需要额外进行主链压力测试或采集 block weight 使用率。

### 6.4 MaxSubmissions=10000 最大吞吐容量

为展示系统在高压提交下的容量上限，补充执行 `MaxSubmissionsPerEpoch=10000` 容量实验。该实验将每条活跃子链压到 10000 次链上接受提交，使用 `pool` 模式发压，并在每个 N 运行前 reset 活跃子链，避免上一轮打满 epoch 后影响下一轮。
...
|---|---------------:|---------:|-----------------:|
| 1 | 88.69 | 1.00× | 88.69 |
| 2 | 172.96 | 1.95× | 168.85 |
| 3 | 264.58 | 2.98× | 252.62 |
| 4 | 316.46 | 3.57× | 246.87 |
| 5 | 389.18 | 4.39× | 334.32 |
| 6 | **396.12** | **4.47×** | 209.62 |

**关键观察**：
- 在 6 条子链并发时，子链总链上接受吞吐达到 **396.12 TPS**，为单链基准的 **4.47×**。
- 主口径采用 `sum_individual_chain_tps`，即各子链链上接受 TPS 求和；灰色辅助线采用“所有活跃链都达到 10000”的保守完成时间口径。
- N=6 未继续接近 6×，主要受 child5 和 child3 的单链拖尾影响，属于局部子链瓶颈，不表示主链被占满。
- child3 原 RPC 入口 f7 会显著拖慢容量实验；最终数据将 child3 发压入口切换为 f8。

> 详细口径、原始数据位置与复现命令见 `capacity-experiment.md`。
...
采集脚本为 `scripts/metrics_fund.js`，主要数据文件为 `docs/experiments/figures/data/exp_e_fund_state_v5.csv`。
...
### 7.3 T=10/T=20 长周期资金锁定

基于相同的 6 任务预算和 `ΣB = 71,502.5 UNIT / Epoch`，进一步给出 T=10/T=20 的计划周期反事实分析。传统方案需要一次性锁定 `T × ΣB`，FishboneChain 仍只锁定当前活跃 epoch 的 `ΣB`。

![长周期任务资金锁定对比](figures/fig_liquidity_horizon.png)

| 计划周期 | FishboneChain 初始锁定 | 传统预锁初始锁定 | 降低倍数 |
|----------|-----------------------:|-----------------:|---------:|
| T=3 | 71,502.5 UNIT | 214,507.5 UNIT | 3× |
| T=10 | 71,502.5 UNIT | 715,025 UNIT | 10× |
| T=20 | 71,502.5 UNIT | 1,430,050 UNIT | 20× |

> 该图是基于已观测单 epoch 预算的计划周期反事实分析，不是额外等待 20 个 epoch 的长跑采样。其目的在于展示资金锁定模型随计划周期 T 的伸缩关系。
...
MaxSubmissions:      10000 per epoch（当前容量实验二进制）
Epoch 结构:          600 Collecting + 20 Syncing blocks
child4 Epoch 时长:   ≈ 62 分钟（620 blocks × 6s）
...
| `docs/experiments/figures/data/exp_scale_state.csv` | Exp D 链状态时序 |
| `docs/experiments/figures/data/exp_capacity_summary.csv` | MaxSubmissions=10000 容量实验汇总 |
| `docs/experiments/figures/data/exp_isolation_summary.csv` | 跨场景隔离对比数据 |
| `docs/experiments/figures/data/exp_liquidity_horizon_summary.csv` | T=3/10/20 资金锁定对比数据 |
| `docs/experiments/figures/data/exp_e_fund_state_v5.csv` | Exp E 资金状态时序 |
| `docs/experiments/figures/fig1_exp_a_bottleneck.png` | 图 1：Exp A 容量瓶颈 |
| `docs/experiments/figures/fig2_exp_b_dedicated.png` | 图 2：Exp B 专用链改善 |
| `docs/experiments/figures/fig3_exp_c_timeline.png` | 图 3：Exp C 时序图 |
| `docs/experiments/figures/fig4_child4_steady.png` | 图 4：child4 稳态 |
| `docs/experiments/figures/fig5_epoch1_snapshot.png` | 图 5：Epoch1 快照 |
| `docs/experiments/figures/fig6_linear_scaling.png` | 图 6：线性扩展图 |
| `docs/experiments/figures/fig_capacity_scaling.png` | 最大吞吐扩展能力 |
| `docs/experiments/figures/fig_isolation_comparison.png` | 跨场景隔离效果对比 |
| `docs/experiments/figures/fig_liquidity_horizon.png` | T=3/10/20 长周期资金锁定对比 |
| `docs/experiments/figures/fig7a_liquidity_ratio.png` | 图 7a：资金锁定比例 |
| `docs/experiments/figures/fig7b_capital_capacity.png` | 图 7b：资金容量对比 |
