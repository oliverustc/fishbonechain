...
- **Stage 6 确定性 IMT fixture 耦合**：`business-fixture` 的 RO proof 已从随机 Merkle 树切换为确定性 IMT fixture（depth 10，leaf 0 = `masked_value_hash`，其余 1023 个 padding leaves 和 4 个 decoy roots 均由 fixture metadata 确定性派生）。`business_input_hash` 扩展包含 `dataset_id`、`field_name`、`depth`、`leaf_index`、`root_list_index`（字符串使用 4-byte LE 长度前缀编码）。这是确定性 fixture 耦合，不是完整生产 IMT；完整 IMT membership、subset/substr、链上 verifier 和 trustless bridge 仍是后续工作。
