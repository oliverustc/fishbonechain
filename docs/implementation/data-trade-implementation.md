...
- **Stage 6 确定性 IMT fixture 耦合**：`business-fixture` 的 RO proof 已从随机 Merkle 树切换为确定性 IMT fixture（depth 10，其余 1023 个 padding leaves 和 4 个 decoy roots 均由 fixture metadata 确定性派生）。这是确定性 fixture 耦合，不是完整生产 IMT。
- **Stage 7 结构化 IMT membership lite**：`business-fixture` 的 IMT 模型升级为四层结构化 Merkle 树（Entry → Dataset → Aggregate → Published root），各层使用独立 domain-separated 确定性 padding。RO proof 的 published leaf 是 aggregate root。`business_input_hash` 扩展包含 `record_id`、`schema_version` 和各层 depth/index 字段。这是 lite 原型，不是动态生产 IMT；完整 IMT membership 的 Entry/Dataset/Aggregate 层语义已就位，subset/substr、链上 verifier 和 trustless bridge 仍是后续工作。
- **Stage 8 动态数据集与请求模型**：新增 `fishbone-zk make-witness` 命令，从 dataset/request JSON 生成 `RangeWitness`。dataset 支持多 record、多 `uint64` field（含 salt/mask），request 支持 range 约束。`BuildRangeWitness` 执行跨文档一致性校验（dataset_id、record_id、field_name、range）。两个 demo dataset（factory_sensors、vehicle_telematics）+ 三个 request fixture（含 out-of-range reject）通过 E2E smoke。仍为 range-only，不涉及 subset/substr、前端、runtime 或 artifact schema 变更。
...
- 链上事件索引与状态同步：详见 [chain-event-indexer-state-sync.md](chain-event-indexer-state-sync.md)（Stage 17）
