...
为了评估安全可扩展的多链数据流通平台在复杂数据流通场景中的性能表现，本文基于 Substrate 实现了 FishboneChain 原型系统。原型系统包含一条主链和多条子链，其中子链负责处理高频数据流通请求，主链负责维护全局状态、接收子链在 epoch 结束时提交的摘要信息，并执行跨链协调和结算相关操作。实验运行在 18 台虚拟机组成的集群上，每台虚拟机配置 8 vCPU、16 GB 内存和约 97 GB 磁盘，节点之间通过千兆内网连接，操作系统为 Ubuntu 22.04 LTS。主链和子链均采用多验证节点部署，并使用 BABE 进行区块生产。

实验负载来自四类具有代表性的数据流通工作负载，包括快递配送、交通感知、医疗标注和传感器网络。这些负载在提交频率、任务数量、奖励预算和数据体量方面存在明显差异，其中交通感知和传感器网络代表高频小额数据流通请求，医疗标注代表低频高价值数据交易请求。该设置可以覆盖高并发、多角色、需求复杂的数据流通场景，也可作为可验证脱敏数据交易和跨链状态证明等上层应用的压力模型。为展示不同架构下的性能差异，本文设置了两类对比场景：第一类是共享链场景，即多类数据流通请求在同一条链上竞争同一提交容量；第二类是多子链隔离场景，即不同类型的请求被部署到独立子链上，分别使用各自的链上容量。对于资金效率评估，本文将 FishboneChain 的 FMC 机制与传统预锁方案进行对比。传统预锁方案要求在任务开始前一次性锁定整个计划期的预算，而 FMC 仅锁定当前 epoch 所需预算，并在任务完成后释放资金。
...
进一步地，本文从同等预算下的任务承载能力评估 FMC 的资金效率。图 4 使用与图 3 相同的 6 个数据流通任务作为一个任务组，比较传统预锁方案和 FMC 在相同资金投入下能够支持的等价任务组数量。对于 3 个 epoch 的计划期，传统预锁方案需要将全部资金用于支持 1 组任务，而 FMC 每次仅锁定一个 epoch 的预算，因此相同资金可支持 3 组等价任务。当计划期扩展至 10 个和 20 个 epoch 时，传统方案仍只能支持 1 组任务，而 FMC 可分别支持 10 组和 20 组任务。

![](../../docs/experiments/figures/fig7b_capital_capacity_v2.png)

图 4 同等预算下可支持的任务组数量

图 3 和图 4 从时间维度和预算维度共同说明了 FMC 的资金流动性优势。图 3 表明 FMC 可以在任务运行过程中动态释放资金，图 4 则说明这种释放机制能够转化为更强的任务承载能力。产生该结果的根本原因是两种方案的锁定资金增长方式不同：传统预锁方案的锁定资金量随计划期长度线性增长，而 FMC 的瞬时锁定资金主要由当前 epoch 的任务预算决定，与完整计划期长度无关。因此，当计划期变长或平台同时运行更多数据流通任务时，FMC 能够显著降低资金占用，并提升同等资金规模下可启动的任务数量。综合上述实验，第三个研究点提出的多链数据流通平台能够为前两个研究点提供可扩展的运行基础：一方面，不同类型的数据交易、状态证明和流通任务可以通过子链隔离获得独立处理能力；另一方面，主链仅承担聚合状态、跨链协调和结算功能，从而避免成为高频业务瓶颈。
